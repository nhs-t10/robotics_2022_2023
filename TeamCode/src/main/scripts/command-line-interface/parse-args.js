...
function cascadeDefaults(schema, out) {
    for(const x in schema) {
        if (!(x in out)) out[x] = schema[x].default || schema[x].value;
    }
}
...
function processLongFlag(schema, arg, out) {
    ...
    if (!schema[key]) return errorNoFlag(key);
    
    const castedValue = castToSchema(schema[key].value, val);
    
    if (key in out) out[key] = [].concat(out[key], castedValue)
    else out[key] = castedValue;
}
...
function castToSchema(typeDesired, arg) {
    switch(typeof typeDesired) {
        ...
        case "object":
            if(typeDesired instanceof Array) return typeDesired.concat(arg);
            else throw new Error("Bad schema type");
        default: return arg;
    }
}
